...
const Editable = ({changeTitle, save, remove, note, editMode, setEditMode}) => {
  ...
  return (
    <>
        ...
        <input required value={title} onChange={e => setTitle(e.target.value)} placeholder='New Title...' className='text-black p-1 rounded-xl w-full max-w-[10ch] lg:max-w-[40ch]'/>
        <i onClick={e => changeTitle(e, title)} className="fa-solid fa-check dark:bg-black p-1 rounded-full border-2 duration-300 hover:bg-gray-300 dark:hover:bg-white cursor-pointer"></i>
        <i onClick={e => setEditMode(prev => !prev)} className="fa-solid fa-xmark dark:bg-black p-1 rounded-full border-2 duration-300 hover:bg-gray-300 dark:hover:bg-white cursor-pointer"></i>
        </form>
        </>
        }
        {!editMode &&
